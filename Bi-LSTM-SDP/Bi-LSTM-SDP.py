...
reg = l1_l2(.01, .01)
l_lstm = Bidirectional(LSTM(64, dropout=.25, recurrent_dropout=.25,
                            kernel_regularizer=reg), name='bi_lstm')(emb_concat)

preds = Dense(19, activation='softmax', name='predictions')(l_lstm)
...
tbCallBack = TensorBoard(log_dir='../dumps/logs/Bi-LSTM-SDP',
                         histogram_freq=1,
                         write_graph=True,
                         write_grads=True,
                         batch_size=64)
