...
def load_and_process(fname, nlp):
    ...
    print('get shortest dependency path between e1 and e2')
    df['shortest_dependency_path'] = df.apply(get_shortest_dependency_path, axis=1)
    df = df.dropna()
    df['sdp_tokens'] = df.apply(lambda row: [token.lower_ for token in row.shortest_dependency_path], axis=1)
    df['sdp_pos'] = df.apply(lambda row: [token.pos_ for token in row.shortest_dependency_path], axis=1)
    df = df.drop('shortest_dependency_path', axis=1)
    print('\t{} seconds'.format(time() - init))
...
def get_shortest_dependency_path(row):
    ...
    try:
        ...
        print('No path between e1 and e2 at index {}.'.format(row.name))
        return None

    shortest_dependency_path = [row.spacy_doc[node] for node in shortest_path]
...
if __name__ == '__main__':
    ...
    init = time()
    with gzip.open('../dumps/semeval2010_task8_train-bi_lstm_sdp.pkl.gz', mode='wb', compresslevel=9) as f:
        pickle.dump(train, f, protocol=3)
    print('\t{} seconds'.format(time() - init))

    print('Saving test set...')
    init = time()
    with gzip.open('../dumps/semeval2010_task8_test-bi_lstm_sdp.pkl.gz', mode='wb', compresslevel=9) as f:
